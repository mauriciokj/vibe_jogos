...
### 2026-09-09 — Obstáculos publicados e verificados

- Código `b636d1e`, release ativa `20260909T222714Z-b636d1e`, GitHub atualizado. Ativação normal com guarda de corridas respeitada. Serviços Asfalto/Catálogo/Caddy ativos, endpoint confirma protocolo12.
- QA HTTPS nos dois domínios passou: árvores nos quatro climas, salto/queda/faixa livre, travessias de feno/tatu, filas de cinco carros nos dois sentidos, rampas com zero cargas, pouso, desktop/celular e consulta ao histórico real. Capturas públicas inspecionadas, sem erros JS/console.
- Smoke multiplayer real: sala privada, dois humanos em domínios diferentes e seis bots, quatro rampas no estado gerado pela VPS, movimento por comandos e saída dos dois participantes. Contextos descartáveis em ?test, sem login, visitantes artificiais ou resultados fictícios no ranking público.
- 133 unitários, nove VPS, build/export, browser local com saltos compartilhados/reconexão, regressões de áudio/armas/empinadas e quatro iterações da skill passaram. Garagem, saldo, recordes pessoais, OAuth, bases e homepage preservados; classificação anterior disponível em Histórico. Concluído.

### 2026-09-09 — Campeonato publicado e verificado

- Código `881cdc3`; release `20260909T231601Z-881cdc3` ativa e GitHub atualizado. Modo individual opcional com cinco etapas/20 corridas: Dia→Entardecer→Noite→Chuva por estrada. Tabela a cada corrida, pontos 10/6/4/3/2/1, top3 para avançar e pontos zerados na nova etapa. Eliminação exige recomeçar na Costa, preservando garagem/créditos.
- Oficina somente antes/entre etapas; moto/equipamento inscritos ficam fixos nas quatro corridas e danos continuam, inclusive nos rivais. Nenhum DNF pontua; abandono explícito na pausa. Apuração continua a simulação real dos rivais. Resultados pagam créditos/recorde pessoal uma vez; corrida livre e multiplayer preservados. Progresso e checkpoint com posição/danos/nitro/cargas seguem o save e a conta Google.
- 141 unitários, nove VPS, build/export, browser local de 20 corridas e regressão de chegada solo/multiplayer passaram. API autenticada/SQLite descartável validou checkpoint grande em dois clientes, idempotência e recusa de revisão antiga. Etapa completa dirigida pela simulação normal confirmou desgaste até quebra e zero pontos nas corridas não concluídas.
- Skill executou controles reais com fixture de aproximação: chegada, tabela, botão de próxima e largada do Entardecer com dano mantido. Capturas/state inspecionados. QA HTTPS passou nos dois domínios, cobrindo as 20 corridas, classificação/eliminação, garagem, reload durante corrida e menu/tabela em 390/320 px. Imagens públicas revisadas em `output/championship/public` e `public-root` na origem, sem erros JS/console. Contextos descartáveis em ?test, sem incrementar visitas, login real ou resultados fictícios no ranking.
- Ativação guardada, sem forçar interrupção. Asfalto/Catálogo/Caddy ativos, protocolo12 e activeRaces=0 confirmados após publicação. Ranking regras2, configuração OAuth, bases, homepage e demais jogos preservados. Documentação em `sources/asfalto-bruto/docs/campeonato.md`. Concluído; registro documental posterior não exige redeploy.

### 2026-09-09 — Card de destaque do campeonato

- Código `b795b2c`; release `20260909T232741Z-b795b2c` publicada. Acesso virou um card quadrado184px com troféu SVG central, título Campeonato e 5 etapas ·20 corridas; à direita da garagem/conta/visitas, conforme referência do usuário. Celular140/124px e indicação Retomar/etapa. Layout flui sem sobrepor as pistas, inclusive com os textos da conta/contador.
- TypeScript/build/export e nove testes VPS passaram. Skill verificou seis telas (1830,1440,1024,390,320 e844 em paisagem), clique no próprio SVG, chegada/tabela/próxima corrida, integridade preservada, retomada e acionamento por Enter. Imagens locais e públicas revisadas em `output/champ-card` na origem. Sem erros JS/console; QA público descartável em ?test, sem visitas artificiais, contas ou resultados falsos no ranking.
- HTTPS dos dois domínios entrega o mesmo HTML e novos assets. Serviços Asfalto/Catálogo/Caddy ativos, guarda respeitada e zero corridas ativas no check. Alteração de apresentação: física, regras, salvamento e protocolo12 preservados. Concluído; registro documental posterior não exige redeploy.

### 2026-09-09 — Reparo de moto zerada e avisos no campeonato

- Código `ec57358`; release `20260909T235038Z-ec57358` publicada com guarda de corridas respeitada. Moto em0 bloqueia a largada antes de consumir corrida ou avançar etapa. Botão Reparar moto disponível em0 mesmo dentro das quatro corridas, custo normal400; preserva pontuação/histórico, equipamento, nitro e dano dos rivais. Oficina completa continua antes/entre etapas.
- Abaixo de20%, aviso destacado antes da largada;1–19% permite correr e não libera reparo no meio da etapa. Durante a corrida, aviso uma vez. Retomada de saves quebrados permite reparar; checkpoints antigos zerados na contagem de largada não perdem a corrida. Resultados pendentes são liquidados uma vez antes do conserto.
- 145 unitários, nove VPS, TypeScript/build/export, browser local do campeonato e regressão da chegada/navegação solo e multiplayer passaram. Skill executou quebra→reparo→chegada→próxima corrida, com imagens/texto inspecionados. Testes de saldo insuficiente, limites exatos, identidade da moto inscrita e checkpoint/cloud passaram.
- QA HTTPS passou nos dois domínios, percorrendo20 corridas e reproduzindo Costa vencida com0→largada bloqueada→garagem→Serra100, quebra na Serra→reload→reparo400 e largada com15%+aviso. Interface desktop/390/320 revisada em `output/champ-repair/public` e `public-root` na origem, sem erros JS/console. Contextos descartáveis em ?test, sem login, visitas artificiais ou resultados falsos no ranking.
- Serviços Asfalto/Catálogo/Caddy ativos; protocolo12 e zero corridas ativas confirmados após publicação. Física, ranking regras2, OAuth, bancos, homepage e demais jogos preservados. Concluído; registro documental posterior não exige redeploy.

### 2026-09-09 — Rivais na largada e policial na chegada publicados

- Código `eb921e0`, release `20260910T003203Z-eb921e0` ativa. Rivais com moto0 recebem o reparo permitido antes da próxima corrida do campeonato; desgaste parcial e eliminações já registradas continuam. Checkpoints antigos em countdown corrigidos. Nas20 largadas de corrida livre verificadas, todos os rivais saíram.
- Policial próximo na chegada estaciona, desce e alterna golpes pelos pilotos presentes; moto vazia/giroflex, passos, reação visual e som de impacto. Animação de resultado preserva classificação, integridade, saldo e a corrida de quem ainda disputa no multiplayer. Pórtico mais alto deixa a cena visível em celular e desktop.
- 149 unitários, nove VPS, TypeScript/build/export passaram. Browser local de campeonato cobriu20 corridas e quatro rivais quebrados→nova largada com todos andando. Browser de chegada cobriu quatro telas, fases da cena, oito alvos, reset e resultados/snapshots intactos, além de dois clientes online. Skill usou cliente original com fixture de aproximação e tempo avançado em blocos para evitar lentidão de chamadas por frame; ações, screenshots e estados revisados em output/grid-police/skill-batched, sem erros. Visitas locais simuladas porque o catálogo não estava ligado no ambiente de teste.
...
- Publicado commit `2c1e6fd0d703364652b9cde33757a28a6f1c917c`, release `20260925T152039Z-2c1e6fd`, com backup concluído e proteção de corridas respeitada. Serviços Asfalto, catálogo e Caddy ativos; Beta1.7.0/protocolo19 confirmado nos dois domínios. Assets públicos idênticos ao build: JS `index-B2pMWC3c.js` SHA256 `328515cd15d5947ab5e4334cf360d666753a4bb3e21ac4952f70b398a05d7aec`; CSS `index-CVqRNDtE.css` SHA256 `55f3e4b718b0d533c214d4da2d757bcb92cda0fc465a2754c2a145f8a5be2b71`.
- QA HTTPS aprovado: golpes reais com bônus repetidos de50 por rival e500 exclusivos por policial, soma por colocação, resultado, recarga e abandono em desktop/390/320px. Assets publicados com autoridade/SQLite locais descartáveis validaram três clientes, quatro corridas na mesma sala, pagamento único de contas/convidados, valores por posição, votos, pista/moto, anfitrião e reconexão. Sem erros JS/console; capturas de resultados e sala inspecionadas.
- Sala privada real na VPS passou com dois convidados nos dois domínios, sete motos, equipamentos, configuração, largada e reload/reconexão. Ambos saíram ao final. Nenhuma conta, ranking ou visita artificial em produção. Cliente da skill aprovado no HTTPS:332 ticks,20,97m/s, captura/estado inspecionados. Artefatos na origem em `output/deploy-beta-1.7.0`; navegadores e serviços de QA encerrados. Publicação concluída; registro documental posterior não requer redeploy.
