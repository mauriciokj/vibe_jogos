...
- Publicado f86f92d na release20260912T173151Z-f86f92d, somente VPS. Asfalto/Catálogo/Caddy ativos; Beta1.4.1/protocolo16 nos dois domínios. JS index-BIarpOpB.js SHA256 15465635daa1553a48ed435f5fd4321a20c0940658c4583ef5761ed83b66dcfe e CSS index-BQ3uL92L.css idênticos ao build local.
- QA HTTPS confirmou versão/histórico, cinco posições consecutivas perto da largada da Costa, contato/redução de velocidade/saída, Serra/Porto e celular em pé/deitado. Imagens públicas conferidas, proteção contínua até a borda, sem erros JS/console. Apenas convidado isolado com ?test, sem contas/rankings/visitas artificiais. Nenhum status/deployment Vercel no commit. QA4397 encerrado; protótipo4390 não alterado. Concluído; registro posterior não requer redeploy.


## 2026-09-14 — Beta1.4.2: seleção da tela e controles presos no celular
- Usuário validou a correção dos guard rails na Beta1.4.1. Novo relato de seleção de textos e comandos presos no celular; navegador específico solicitado, sem resposta até esta validação.
- Canvas, HUD e botões impedem seleção, callouts e gestos de navegação durante a corrida; prefixos Safari e bloqueio de selectstart/contextmenu/dragstart restritos à superfície do jogo. Textos de diálogos e campos de formulário mantêm seleção, menu de contexto e rolagem.
- Novo bindPointerControl associa um dedo a cada analógico/botão, acompanha movimentos e término no window, filtra releases de outros dedos, trata cancelamento/perda de captura e libera a captura ao reiniciar controles. Teclado e botões por toque têm estado separado. Golpes mostram enquanto estão pressionados. Pause/blur/visibility/pagehide/rotação limpam estados e exigem novo toque. Multiplayer recebe neutralização/freio imediatamente ao limpar, sem esperar uma animação que pode parar em segundo plano.
- Novo test:mobile passou em Chromium e WebKit: proteção contra seleção, vários ponteiros, soltar fora, cancelamento, perda de captura, retorno de interrupções, nitro por toque único, campos editáveis, garagem e paisagem. Chromium também testou três dedos nativos simultâneos, segurar1,25s, liberar só um dedo, cancelar e tocar novamente sem seleção/zoom/rolagem. WebKit no macOS não implementa -webkit-touch-callout, então essa propriedade é conferida somente quando suportada; não substitui validação num iPhone físico.
- QA de bicicleta passou: pedaladas/toque versus segurar, freio, chegada a pé, campeonato e conta em outro dispositivo. QA de barreiras/multiplayer passou com dois humanos/seis bots, reconexão e captura do pacote que neutraliza comandos sincronamente no blur. Sem erros JS/console. TypeScript/build/export e9 testes VPS aprovados. Capturas desktop/mobile/WebKit conferidas. Preparando publicação somente na VPS, protocolo16/ranking3, sem mudanças na física ou nos saves.

- Publicado código7efa776 na release20260914T200128Z-7efa776 somente na VPS. Asfalto/Catálogo/Caddy ativos; Beta1.4.2/protocolo16 nos dois domínios. JS index-CB7-gbCp.js (SHA256 9c8c276d4466c4e0a74c129012a862904a1337b704bcc81b1376b4f01562fbf4) e CSS index-CRM9zmZm.css (SHA256 625daf0e82004042f074ed36c5192ff8edd3cf64ac6d066df21bd80740dfe26f) idênticos ao build.
- QA HTTPS test:mobile repetido em Chromium/WebKit passou com imagens públicas conferidas e zero erros JS/console. Convidados isolados, ?test e serviços de conta/visitas simulados no navegador; sem contas/resultados/visitas artificiais. Skill final passou com525 ticks/51,78m/s e imagem idêntica à conferida antes do ajuste de envio imediato online. Testes nativos usaram CDP com touchEnd do dedo específico, confirmado por eventos reais; WebKit cobre ciclo de ponteiros e toque único, não gestos de sistema de um iPhone físico. Nenhum status/deployment Vercel no commit. QA4397/4398/4367/4394 encerrados; protótipo4390 não alterado. Aguardando retorno do usuário no aparelho real; registro final não exige redeploy.


## 2026-09-14 — Asfalto Bruto Beta1.4.3: toque nativo e recuperação do acelerador
- Corrige zoom residual e acelerador preso relatados após1.4.2. Reprodução por falha injetada: cancelar toque sem pointercancel deixava comando ativo. Analógicos/golpes usam dedos nativos quando disponíveis, reconciliam a lista completa e recuperam nova pressão inclusive com ID reutilizado. Mouse/caneta mantêm captura.
- Cancelamento de gestos no canvas/controles e gesture do Safari durante corrida, preservando cliques de nitro/pausa, campos e garagem. Sem mudar física, save, protocolo16 ou ranking3.

- Validação final aprovada: test:mobile em Chromium/WebKit com término/cancelamento nativo, término omitido, ID reutilizado, freio após falha, dedos independentes, interrupções, toque rápido, pinça no Chromium, nitro/pausa e formulários. Nenhum zoom (escala1), seleção/scroll acidental ou erro JS nos cenários testados. Skill passou525 ticks/51,78m/s; screenshots de corrida, celular e paisagem conferidos. Bicicleta/pedaladas e barreiras/multiplayer/reconexão também passaram. TypeScript/build/export e9 testes VPS aprovados.
- Preparando publicação somente na VPS; nenhuma publicação Vercel. WebKit emulado não substitui validação em aparelho iOS físico. Protótipo4390 preservado.
...
- Publicado código d0ad851 somente na VPS, release20260922T210843Z-d0ad851. Asfalto/Catálogo/Caddy ativos. Dois domínios entregam Beta1.4.4/protocolo17, JS index-C4-76xEd.js SHA25645a1dcb2a62bbb25d4523e7e9fe1e1be359b49411837fb36d92c6ee9db1cb031 e CSS index-CRM9zmZm.css SHA256625daf0e82004042f074ed36c5192ff8edd3cf64ac6d066df21bd80740dfe26f iguais ao build.
- QA HTTPS passou em sala privada temporária com dois convidados: sete opções de moto sem posse, Brutal/Falcão, kits distintos (dourada/corrente e azul/garrafa), largada e recarga/reconexão mantendo o equipamento. Capturas desktop/mobile conferidas, sem erros JS/console. Ambos saíram da sala; nenhum login/registro/ranking/contador de visitantes de produção alterado pelo QA. Equipamento oficial de conta validado com SQLite e sessões isoladas localmente. QA4398/4399/4360 encerrados; protótipo4390 preservado. Registro final documental sem redeploy.

## 2026-09-23 — Asfalto Bruto Beta1.5.0: espectador no multiplayer
- Usuário validou a versão local e autorizou publicação. Depois de prisão/explosão, o eliminado acompanha humanos conectados ainda na corrida. C/botão alterna; saída/chegada/desconexão troca automaticamente, e sem alvos aparece o resultado. Cena de prisão preservada, HUD/áudio seguem o observado; nenhum comando de pilotagem do espectador.
- 218 testes automatizados, browser de três jogadores, prisão online, desktop/celular e cliente da skill aprovados. Resultado/conquistas sem duplicação, reconexão e novo jogo individual verificados. Nove testes VPS e export/build TypeScript/Vite aprovados. Fonte, bundle, documentação e novidades atualizados; protocolo17 preservado.
- Revisão da diferença limitada ao Asfalto Bruto e este registro. VPS saudável e sem corridas ativas no check inicial. Ativação com guarda de partidas e QA HTTPS pendentes.
- Publicado código c4317c4 na VPS, release20260923T200105Z-c4317c4. Asfalto/Catálogo/Caddy ativos, Beta1.5.0/protocolo17. Dois domínios confirmam arquivos iguais ao build: JS index-ChMieeKr.js SHA256 f5461a4325c3b7195dedd1b5035c8189cf00b6d423c154ef932e9f649ad93a52; CSS index-Ie2X9Cti.css SHA256 cc6b09eca2444807b70dc39b7f300a6562579359b3572593df473641767c80cd.
- Sala privada real na VPS com dois convidados em domínios diferentes validou largada, equipamento e reconexão; ambos saíram e saúde confirmou activeRaces0. Funcionalidade de espectador validada novamente com assets HTTPS publicados e servidor de simulação isolado via ponte WebSocket: prisão, explosão, C/botões, queda recuperável, saída/fim do alvo, resultados, reconexão e retorno ao individual. Imagens desktop/mobile/paisagem conferidas e zero erros JS/console. Sem contas/rankings/visitas artificiais.
- Fixture de navegador estabilizada: policial removido da simulação após a captura da cena de prisão para não eliminar os demais pilotos parados durante as asserções. Primeira rodada pública teve troca automática do alvo entre asserções; a repetição com isolamento passou sem alteração no produto. Registro e fixture posteriores não exigem redeploy.
- deploymentEnabled:false preservado. Consulta adicional de statuses GitHub indisponível por gh sem autenticação; nenhum deploy Vercel foi solicitado. Publicação concluída.
