import type { Command, RaceCondition, RaceState, SaveData } from '../game/types';
...
export interface RankedRun { id: string; seed: number; save: SaveData; trackId: string; condition: RaceCondition; rules: number; initial?:RaceState; championship?:boolean; cursor?:number; cloud?:CloudSave; }
export interface GarageAction { kind:'bike'|'helmet'|'color'|'weapon'|'knee'|'nitro'|'upgrade'|'repair'|'champRepair'|'champRestart'|'removeWeapon'|'removeKnee'|'preferences'|'reset'; item?:string; muted?:boolean; trackId?:string; condition?:RaceCondition; }
