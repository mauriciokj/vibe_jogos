...
import type { AchievementProgress, AchievementId } from './achievements';
import type { RaceFeats } from './race-feats';
...
export type BikeStyle = 'street' | 'sport' | 'muscle' | 'supermoto' | 'cruiser' | 'chopper' | 'cafe' | 'bicycle';
export type RiderAction = 'kneeLeft' | 'kneeRight' | 'nitro' | 'horn' | 'taunt' | 'wheelie' | 'pedal';
...
export interface RaceResult { rivalKnockdowns?:number; reason: 'finish' | 'wrecked' | 'caught' | 'left' | 'timeout'; policeKnockdowns?:number; stolenPoliceBike?:boolean; achievements?:AchievementId[]; onFoot?: boolean; secretUnlocked?: boolean; arrestCause?: 'fall' | 'stopped'; place: number; time: number; reward: number; hits: number; falls: number; }
