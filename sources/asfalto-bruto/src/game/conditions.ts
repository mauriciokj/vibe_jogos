...
export function conditionTrack(track: Track, value: unknown): Track {
  ...
  const desert=track.id==='deserto';
  const visual={...track,...(condition==='day'?{
    sky:['#287fad','#86cddb','#e2efd0'],land:desert?['#cc9b64','#c19360']:['#719d66','#65905b'],road:['#56646b','#515e65'],
  }:condition==='night'?{
    sky:['#060d23','#142746','#385975'],land:desert?['#534d50','#4a4449']:['#29474a','#244044'],road:['#33434e','#2d3c48'],
  }:{
    sky:['#344d60','#688491','#acbbc0'],land:desert?['#9c8166','#90765d']:['#57786e','#4e6c64'],road:['#3b535e','#344a56'],
  })};
...
function decorHash(seed: number) { let n=seed|0;n=Math.imul(n^(n>>>16),0x45d9f3b);n=Math.imul(n^(n>>>16),0x45d9f3b);return ((n^(n>>>16))>>>0)/4294967296; }
