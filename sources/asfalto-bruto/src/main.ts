...
function resetFinish() {
  finishElapsed=null;$('finish-scene').hidden=true;$('result-modal').classList.remove('finish-result');
}
function beginResult(finished: boolean, place: number) {
  ...
  finishElapsed=0;screen='finish';$('hud').hidden=true;$('finish-scene').hidden=false;
...
function draw() { renderer.render(race, screen === 'menu',localId(),finishElapsed); if (screen !== 'menu' && finishElapsed===null) { updateHUD(); instruments.draw(race,localId()); } }
...
if (testMode) window.__game = {
  ...
  restore: raw => { if(onlineMode)throw new Error('Use o servidor para testar uma corrida online.'); resetFinish();race = restoreSnapshot(raw); settled = false; screen = race.mode === 'finished' ? 'result' : 'race'; paused = false; closeDialogs(); $('menu').hidden = true; $('hud').hidden = false; draw(); },
  start: startRace,
