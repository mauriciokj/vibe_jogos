...
function update() {
  ...
  audio.update(localRider().speed, true, race.policeActive && Math.abs((race.riders.find(r => r.id === 'police')?.z ?? 99999) - localRider().z) < 100, race.time, getBike(localRider().bikeId).style, jumpSoundKey(localRider()),guardRailSoundSide(race.trackId,localRider()),race.trackId==='terra'?'earth':'metal',jumpSoundMaterial(race,localRider()));
...
function frame(now: number) {
  ...
  if(onlineMode) {
    ...
    if(screen==='race'){const p=localRider();audio.update(p.speed,!paused,race.policeActive && Math.abs((race.riders.find(r=>r.profile==='police')?.z ?? 99999)-p.z)<100,race.time,getBike(p.bikeId).style,jumpSoundKey(p),guardRailSoundSide(race.trackId,p),race.trackId==='terra'?'earth':'metal',jumpSoundMaterial(race,localRider()));}
