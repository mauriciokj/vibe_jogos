import './recovery.css';
import { releaseUI } from './releases';
import { raceOutcome } from './game/race-outcome';
import { ARREST_SECONDS, arrestScene } from './game/arrest';
...
import type { GarageAction } from './account/protocol';
...
import { ACHIEVEMENTS, awardRaceAchievements } from './game/achievements';
import { medalIcon, achievementsMarkup, achievementRewardMarkup, type AchievementFilter } from './achievements-ui';
...
import { policeReward, rivalReward, racePayout, POLICE_KNOCKDOWN_REWARD, RIVAL_KNOCKDOWN_REWARD, type RacePayout } from './game/rewards';
import { enteredBike } from './game/police-bike';
import { BICYCLE_ID, POLICE_BIKE_ID, MOTORBIKES, isBicycle, supportsKneeDown } from './game/bikes';
...
import { spectatorTarget, spectatorTargets } from './multiplayer/spectator';
import { PublicRoomBrowser } from './multiplayer/discovery';
import type { RoomView } from './multiplayer/protocol';
import { settleGuestOnlineResult } from './game/save';
...
root.innerHTML = `
  ...
  <div id="ui">
    <section id="menu" aria-label="Menu principal">
      ...
      <div class="menu-main"><div class="eyebrow"><i class="live-dot"></i> OITO PILOTOS. UMA CHEGADA.</div>
        ...
        </div><div class="menu-awards"><button id="championship-btn" aria-label="Campeonato, 5 etapas e 20 corridas"><span id="champ-card-state" class="champ-card-state" hidden></span><span class="champ-card-arrow" aria-hidden="true">↗</span>${icons.trophy}<strong>CAMPEONATO</strong><span class="champ-card-detail">5 ETAPAS · 20 CORRIDAS</span></button><button id="achievements-btn" aria-label="Ver conquistas">${medalIcon}<strong>CONQUISTAS</strong><small id="achievement-count">0 / 25</small></button></div></div>
      </div>
    ...
    <section id="hud" hidden aria-label="Informações da corrida">
      <div id="spectator-bar" hidden><div role="status"><small id="spectator-reason"></small><strong id="spectator-name"></strong></div><div class="spectator-actions"><button id="spectator-next" class="secondary"><kbd>C</kbd> TROCAR CÂMERA</button><button id="spectator-result" class="text-button">VER RESULTADO</button></div></div>
      ...
      <div class="countdown" id="countdown"><strong id="count-number">3</strong><span id="count-instruction">PREPARE-SE · SEGURE W OU ↑</span></div>
  ...
  <dialog id="help-modal" aria-labelledby="help-title"><div class="dialog-header"><div><div class="eyebrow">ANTES DE DAR A PARTIDA</div><h2 id="help-title">Conheça as regras da rua.</h2></div><button class="close-btn" data-close="help-modal" aria-label="Fechar">×</button></div><div class="dialog-body"><div class="help-grid">
    ...
    <div class="control keyboard-only">Buzinar <span class="keys"><kbd>B</kbd></span></div><div class="control keyboard-only">Provocar <span class="keys"><kbd>Q</kbd></span></div><div class="control">Nitro <span class="keys"><kbd>N</kbd></span></div><div class="control">Pausar <span class="keys"><kbd>Esc</kbd></span></div><div class="control">Trocar câmera após perder no multiplayer <span class="keys"><kbd>C</kbd></span></div><div class="control">Tela cheia / som <span class="keys"><kbd>F</kbd><kbd>M</kbd></span></div>
  ...
  <dialog id="online-modal" class="online-modal" aria-labelledby="online-title">
    ...
    <div class="dialog-body">
      ...
      <div id="online-lobby" hidden><p id="online-visibility" class="online-visibility"></p><div class="lobby-heading"><div><span class="online-label">CONVIDE PELO CÓDIGO</span><div class="room-code" id="online-code"></div><button class="text-button" id="online-copy">COPIAR CONVITE ↗</button></div><div class="lobby-clock"><strong id="online-clock">60</strong><span id="online-clock-label">SEGUNDOS PARA LARGAR</span></div></div>
        <div class="lobby-meta"><span id="online-track-name"></span><span id="online-count">1 / 8 PILOTOS</span></div>
        <div class="lobby-settings"><div><label class="online-label" for="lobby-track">PISTA DA SALA</label><select id="lobby-track" class="online-input">${RACE_ROUTES.map(r=>`<option value="${r.id}">${r.name}</option>`).join('')}</select><small id="lobby-host-hint"></small></div><div><label class="online-label" for="lobby-bike">SUA MOTO</label><select id="lobby-bike" class="online-input"></select><small>Motos de fábrica · equipamento da sua garagem</small></div></div>
        <div id="online-members" class="lobby-members"></div>
...
const accounts = new AccountClient(next=>{
  ...
  syncSound();renderMenu();if(screen==='menu'&&!startingRace)makeAttract();
...
let screen: 'menu' | 'race' | 'finish' | 'result' | 'spectate' = 'menu';
let spectatedId: string | null = null;
let finishElapsed: number | null = null;
let arrestSource:RaceState|null=null;
...
let achievementRoom:RoomView|null=null,achievementRiderId='';
function bankGuestOnlineAchievements(){const last=achievementRoom;achievementRoom=null;if(accounts.cache||settled||!last?.race||!last.race.multiplayer)return;const s=structuredClone(last.race),r=s.riders.find(r=>r.id===achievementRiderId);if(!r)return;if(!s.multiplayer!.results[r.id])finishRider(s,r,'left');settleGuestOnlineResult(save,s,r.id,last.members.find(m=>m.id===r.id)?.entryId ?? r.id);saveNow();}
function showAchievements(){ $('achievements-modal').innerHTML=achievementsMarkup(save,achievementFilter);if(!$<HTMLDialogElement>('achievements-modal').open)$<HTMLDialogElement>('achievements-modal').showModal(); }
function showAchievementRewards(ids:import('./game/achievements').AchievementId[]=[]){const html=achievementRewardMarkup(ids);if(html&&!$('result-modal').querySelector('.achievement-rewards'))$('result-modal').insertAdjacentHTML('beforeend',html);}
function bankGuestAchievements(){if(!onlineMode&&!race.multiplayer&&!accounts.cache&&!settled&&screen==='race'&&race.mode==='racing'){finishRider(race,localRider(),'left');save.cash+=racePayout(race.trackId,race.result!).total;awardRaceAchievements(save,race,localId(),championshipMode?'championship':'solo');settled=true;saveNow();}}
...
const online = new OnlineClient({
  ...
  left: () => { bankGuestOnlineAchievements();onlineMode=false;onlineRaceStarted=false;lastOnlineRound=-1;showMenu();void accounts.refresh(); },
});
...
function viewingId() { return screen==='spectate' && spectatedId ? spectatedId : localId(); }
function viewingRider() { return race.riders.find(r=>r.id===viewingId()) ?? localRider(); }
...
async function refreshOnlineGarage(){
  const id=accounts.cache?.account.id;if(!id)return;
  try{
    const session=await accounts.api('/session');
    if(session.account?.id!==id || accounts.cache?.account.id!==id || accounts.session?.account?.id!==id)return;
    // Refresh balances and unlocks without replacing the active room/camera.
    accounts.acceptCloud(session.cloud,false);save=structuredClone(accounts.cache.save);
  }catch{/* The next result/lobby/exit retries the authoritative refresh. */}
}
...
function renderOnlineBikes(preferred=$<HTMLSelectElement>('online-bike').value){
  const equipment=onlineEquipmentSave(),available=BIKES.filter(b=>!b.singlePlayerOnly && (!b.secret || accounts.session?.account && equipment.owned.includes(b.id)));
...
$<HTMLSelectElement>('lobby-track').addEventListener('change',()=>{const route=routeFromId($<HTMLSelectElement>('lobby-track').value);online.configure({trackId:route.track.id,condition:route.condition.id});});
$<HTMLSelectElement>('lobby-bike').addEventListener('change',()=>{const bikeId=$<HTMLSelectElement>('lobby-bike').value;online.configure({bikeId,loadout:onlineLoadout(bikeId)});});
...
function drawLobbyClock() {
  ...
  $('online-clock-label').textContent=count<2?'AGUARDANDO OUTRO PILOTO':room.locked?'LARGADA FECHADA':room.manualStart?'AGUARDANDO CONFIRMAÇÃO':'SEGUNDOS PARA LARGAR';
}
function receiveOnlineRoom(room: RoomView) {
  achievementRoom=room;achievementRiderId=online.id;
  if(!onlineMode)return;
  if(room.previous && !accounts.cache){
    const member=room.previous.members.find(m=>m.id===online.id);
    if(member && settleGuestOnlineResult(save,room.previous.race,online.id,member.entryId ?? member.id))saveNow();
  }
  if(lastOnlineRound!==(room.round ?? 0)){
    lastOnlineRound=room.round ?? 0;onlineRaceStarted=false;settled=false;lastOnlineEventTick=-1;lastStandingsTick=-Infinity;
    resetFinish();clearControls();closeDialogs();
    if(accounts.cache)void refreshOnlineGarage();
  }
  if(room.phase==='lobby') {
    if(onlineRaceStarted || screen!=='menu'){
      resetFinish();clearControls();closeDialogs();onlineRaceStarted=false;settled=false;paused=false;screen='menu';lastOnlineEventTick=-1;lastStandingsTick=-Infinity;
      $('menu').hidden=false;$('hud').hidden=true;$('online-hud').hidden=true;audio.update(0,false,false,0);
      race=createRace(room.trackId,save,undefined,room.condition);
    }
    if(!$<HTMLDialogElement>('online-modal').open)$<HTMLDialogElement>('online-modal').showModal();
    ...
    const track=$<HTMLSelectElement>('lobby-track'),bike=$<HTMLSelectElement>('lobby-bike');
    track.value=raceRoute(room.trackId,room.condition).id;track.disabled=room.hostId!==online.id || room.locked || online.status!=='connected';
    const available=BIKES.filter(b=>!b.singlePlayerOnly && (!b.secret || b.id===me?.bikeId || accounts.session?.account && onlineEquipmentSave().owned.includes(b.id)));
    const options=available.map(b=>`<option value="${b.id}">${b.name} · ${b.class}</option>`).join('');if(bike.innerHTML!==options)bike.innerHTML=options;
    bike.value=me?.bikeId ?? 'ferro';bike.disabled=room.locked || online.status!=='connected';
    $('lobby-host-hint').textContent=room.hostId===online.id?'Você é o anfitrião. Trocar a pista reinicia as confirmações.':`Pista escolhida por ${present.find(m=>m.id===room.hostId)?.name ?? 'outro piloto'}.`;
    ...
    $('online-lobby-hint').textContent=present.length<2?(room.public?'Sua sala aparece na busca. Aguardando pelo menos mais uma pessoa para largar.':'Precisamos de pelo menos 2 pessoas para largar. Convide um amigo.'):room.locked?'Prepare-se! A entrada de novos pilotos está fechada.':room.manualStart?'Escolham pista e motos. A corrida só começa quando todos estiverem prontos.':'Todos prontos? A contagem cai para 5 segundos. Caso contrário, a corrida começa ao zerar o tempo.';
  ...
  if(!room.race)return;
  const mine=room.race.riders.find(r=>r.id===online.id);if(!accounts.cache && mine && recordOnlineNitro(save,mine,room.members.find(m=>m.id===online.id)?.entryId ?? online.id))saveNow();
  race=online.view()!;
  ...
  if(room.race.tick>lastOnlineEventTick) {
    for(const event of room.race.events)if((event.tick ?? room.race.tick)>lastOnlineEventTick && (event.actor===viewingId() || event.target===viewingId() || event.type==='police' || event.type==='horn' && Math.abs((race.riders.find(r=>r.id===event.actor)?.z ?? Infinity)-viewingRider().z)<100)) {
  ...
  if(screen==='spectate')syncSpectator(room);
}
function showOnlineResult(room: RoomView) {
  ...
  if(!settled) {
    ...
    $('result-modal').innerHTML=`<div class="result-top"><div class="eyebrow">MULTIPLAYER / ${conditionName(room.condition).toUpperCase()} / SALA ${room.code}</div><h2 id="result-title" class="result-title">${title}</h2><p class="result-sub">${result.reason==='finish'?'Você cruzou a linha de chegada.':raceOutcome(result.reason,{...result,exploded}).description}</p><p id="online-result-status" class="online-note"></p></div><div id="online-result-table" class="result-table"></div><div class="result-actions"><button id="online-menu-btn" class="primary">VOLTAR AO MENU ↗</button></div>`;
    $('result-modal').querySelector('.result-actions')!.innerHTML=resultButtons(true,!!nextRaceRoute(room.trackId,room.condition));
    $('result-modal').querySelector('.result-actions')!.insertAdjacentHTML('beforebegin','<p id="online-continuation-status" class="finish-room-hint" role="status" aria-live="polite"></p>');
    const guest=accounts.cache?undefined:settleGuestOnlineResult(save,room.race!,online.id,room.members.find(m=>m.id===online.id)?.entryId ?? online.id);
    const unlocked=accounts.cache?result.secretUnlocked===true:guest?.unlocked;
    const achievements=accounts.cache?result.achievements:guest?.achievements;
    resultPayout=racePayout(room.trackId,result);
    if(accounts.cache)void refreshOnlineGarage();else saveNow();
    $('result-modal').querySelector('.result-top')!.insertAdjacentHTML('afterend',`<div class="result-stats"><div><small>SEU TEMPO</small><b>${clockString(result.time)}</b></div><div><small>${result.reason==='finish'?'PRÊMIO DA CORRIDA':result.reason==='left'?'ABANDONO':'AJUDA DA OFICINA'}</small><b>${money(resultPayout.baseReward)}</b></div><div><small>RECOMPENSA TOTAL</small><b class="prize">+ ${money(resultPayout.total)}</b></div></div>`);
    showKnockdownRewards(resultPayout);
    if(unlocked)showSecretUnlock();showAchievementRewards(achievements);
    if(result.reason!=='wrecked' || !beginSpectating())beginResult(result);
  }
  ...
  const choice=room.members.find(m=>m.id===online.id)?.continuation,present=room.members.filter(m=>m.connected);
  for(const [id,vote,label] of [['online-next-btn','next','PRÓXIMA CORRIDA'],['online-again-btn','again','CORRER DE NOVO'],['online-lobby-btn','lobby','VOLTAR PARA A SALA']] as const){
    const button=$<HTMLButtonElement>(id);if(!button)continue;
    button.disabled=room.phase!=='finished'||online.status!=='connected'||vote!=='lobby'&&(room.continuationCount ?? present.length)<2;button.setAttribute('aria-pressed',String(choice===vote));
    button.textContent=vote==='lobby'?label:`${choice===vote?'✓ ':''}${label} · ${present.filter(m=>m.continuation===vote).length}/${room.continuationCount ?? present.length}`;
  }
  $('online-continuation-status').textContent=room.phase!=='finished'?'Aguarde os outros pilotos terminarem para continuar com o grupo.':choice?`Sua escolha foi registrada. Todos precisam escolher a mesma corrida. Voltar à sala permite ajustar pista e motos.`:'Todos em “Próxima corrida” avançam juntos. “Voltar para a sala” reabre a configuração para o grupo.';
  if(room.phase==='finished' && room.reconnectingCount)$('online-continuation-status').textContent+=` Aguardando ${room.reconnectingCount} piloto(s) reconectar.`;
  if(room.phase==='finished' && (room.continuationCount ?? present.length)<2)$('online-continuation-status').textContent='Apenas você permanece. Volte para a sala e convide outro piloto para continuar.';
  ...
  const targets=spectatorTargets(room,online.id);
  let watch=$<HTMLButtonElement>('online-spectate-btn');
  if(!watch){$('result-modal').querySelector('.result-actions')!.insertAdjacentHTML('afterbegin','<button id="online-spectate-btn" class="secondary" hidden>ACOMPANHAR CORRIDA · C</button>');watch=$<HTMLButtonElement>('online-spectate-btn');}
  watch.hidden=!targets.length;
  setText('finish-skip',result.reason==='caught' && targets.length?'ACOMPANHAR CORRIDA · C':'VER RESULTADO ↗');
}

function selectSpectator(id: string) {
  if(spectatedId!==id){renderer.resetCamera();lastStandingsTick=-Infinity;messageUntil=0;setText('race-message','');}
  spectatedId=id;
  const p=viewingRider();audio.resetRace(jumpSoundKey(p),guardRailSoundSide(race.trackId,p));
}
function beginSpectating() {
  const target=spectatorTarget(spectatorTargets(online.room,online.id),null);
  if(!onlineMode || !target)return false;
  resetFinish();closeDialogs();clearControls();paused=false;screen='spectate';
  $('hud').hidden=false;$('spectator-bar').hidden=false;document.body.classList.add('spectating');
  selectSpectator(target);syncSpectator(online.room!);return true;
}
function syncSpectator(room: RoomView, cycle=false) {
  const targets=spectatorTargets(room,online.id),target=spectatorTarget(targets,spectatedId,cycle);
  if(!target){showSpectatorResult();return;}
  if(target!==spectatedId)selectSpectator(target);
  setText('spectator-name',`ACOMPANHANDO ${viewingRider().name}`);
  const exploded=room.race?.riders.find(r=>r.id===online.id)?.recovery?.phase==='exploding';
  setText('spectator-reason',room.race?.multiplayer?.results[online.id]?.reason==='caught'?'VOCÊ FOI PRESO · ESPECTADOR':exploded?'SUA MOTO EXPLODIU · ESPECTADOR':'SUA MOTO QUEBROU · ESPECTADOR');
  $<HTMLButtonElement>('spectator-next').disabled=targets.length<2;
}
function cycleSpectator() {
  if(screen==='spectate' && online.room)syncSpectator(online.room,true);
  else beginSpectating();
}
function showSpectatorResult() {
  resetFinish();clearControls();paused=false;screen='result';$('hud').hidden=true;closeDialogs();
  audio.update(0,false,false,0);$<HTMLDialogElement>('result-modal').showModal();
}

function showKnockdownRewards(payout:RacePayout|null){
  for(const [kind,label,bonus,unit] of [
    ['rival','RIVAIS DERRUBADOS',payout?.rivalBonus,RIVAL_KNOCKDOWN_REWARD],
    ['police','POLICIAL DERRUBADO',payout?.policeBonus,POLICE_KNOCKDOWN_REWARD],
  ] as const){
    if(!bonus || $('result-modal').querySelector(`.${kind}-bonus`))continue;
    const count=bonus/unit;
    const html=`<div class="record-bonus ${kind}-bonus" role="status"><div><strong>${label}</strong><small>${count} ${count===1?'derrubada':'derrubadas'} × ${unit} moedas</small></div><b>+ ${money(bonus)}</b></div>`;
    const anchor=$('result-modal').querySelector('.result-table, .result-actions');
    if(anchor)anchor.insertAdjacentHTML('beforebegin',html);else $('result-modal').insertAdjacentHTML('beforeend',html);
  }
}

function showPoliceBikeUnlock(){
  if($('result-modal').querySelector('.police-bike-unlock'))return;
  $('result-modal').insertAdjacentHTML('afterbegin','<div class="secret-unlock police-bike-unlock" role="status"><strong>MOTO DA POLÍCIA DESBLOQUEADA!</strong><p>A Patrulha 900 está na sua garagem para corridas individuais e campeonato. Indisponível no multiplayer.</p></div>');
}
function showSecretUnlock(){
  if($('result-modal').querySelector('.secret-unlock'))return;
  $('result-modal').insertAdjacentHTML('afterbegin','<div class="secret-unlock" role="status"><strong>VEÍCULO SECRETO DESBLOQUEADO!</strong><p>A bicicleta Magrela já está na sua garagem. Cada toque no acelerador é uma pedalada.</p></div>');
}
...
function renderMenu() {
  setText('achievement-count',`${save.achievements?.unlocked.length ?? 0} / ${ACHIEVEMENTS.length}`);
  if($<HTMLDialogElement>('achievements-modal').open)$('achievements-modal').innerHTML=achievementsMarkup(save,achievementFilter);
...
async function startRace(championship=false) {
  ...
  // An awaited account request can redraw the old fall after resetFinish().
  // Reset again only after the new/continued race has replaced that state.
...
function pauseGame() {
  if ((screen !== 'race' && screen !== 'spectate') || race.mode === 'finished') return;
...
function syncSoloNitro() {
  ...
  if(used>soloNitroSpent){spendNitro(save,enteredBike(p).bikeId,used-soloNitroSpent);soloNitroSpent=used;saveNow();}
}
...
function queueAction(action: RiderAction) {
  ...
  if(action==='kneeLeft' || action==='kneeRight') {
    if(!supportsKneeDown(p.bikeId)){if(!isBicycle(p.bikeId))toast('Choppers não fazem a manobra de joelho.');return;}
...
function acceleratorTap(at: number) { if(isBicycle(localRider().bikeId)){queueAction('pedal');return;}if(throttleTap.press(1,at))queueAction('wheelie'); }
...
function updateHUD() {
  ...
  const p = viewingRider(), standings=onlineMode?(online.room?.race ?? race):race;
  const order = ranking(standings), place = order.findIndex(r => r.id === viewingId()) + 1;
  const standingZ=standings.riders.find(r=>r.id===viewingId())?.z ?? p.z;
  const bicycle=isBicycle(p.bikeId);
  setText('count-instruction',bicycle?'PREPARE-SE · TOQUES NO ACELERADOR PARA PEDALAR':'PREPARE-SE · SEGURE W OU ↑');
  setText('drive-label',p.recovery?'FRENTE / TRÁS':bicycle?'TOQUE PARA PEDALAR':'ACELERAR / FREAR');
  $('drive-stick').setAttribute('aria-label',p.recovery?'Correr para frente e para trás':bicycle?'Toque repetidamente na parte de cima para pedalar; parte de baixo freia':'Acelerar e frear');
  $('touch-nitro').hidden=bicycle;$('nitro-indicator').hidden=bicycle;
  ...
  setText('wheelie-indicator',bicycle?(p.recovery?'':(p.pedalTime ?? 0)>0?'PEDALANDO':'TOQUE PARA PEDALAR'):`${p.jumpTime!>0?'SALTO':p.wheelieTime!>0?'EMPINANDO':'EMPINADAS'} ${wheeliesLeft(p)}/3`);
  ...
  if (race.tick < lastStandingsTick || race.tick - lastStandingsTick >= 6 || place !== lastStandingsPlace || inCountdown) {
    ...
    $('rival-list').innerHTML = order.slice(start, start + 4).map((r, i) => `<div class="rival-entry ${r.id === viewingId() ? 'me' : ''}"><span>${start + i + 1}</span><span>${escapeHTML(r.name)}</span><span class="gap">${r.out==='caught'?'PRESO':r.out?'FORA':r.id === viewingId() ? '◂' : `${r.z >= standingZ ? '+' : '−'}${Math.round(Math.abs(r.z - standingZ))}m`}</span></div>`).join('');
  }
}
...
function renderGarage() {
  ...
  $('garage-modal').innerHTML = `<div class="dialog-header"><div><div class="eyebrow">SUA OFICINA. SUAS REGRAS.</div><h2 id="garage-title">Garagem</h2></div><button class="close-btn" data-close="garage-modal" aria-label="Fechar garagem">×</button></div><div class="dialog-body">${garageNav(garageTab)}<div class="garage-intro"><div><b>${MOTORBIKES.length} MOTOS E DOIS SEGREDOS.</b><p>Compare os dados de fábrica. As melhorias valem para a moto equipada.</p></div><button class="secondary" id="garage-workshop">MELHORIAS & REPAROS ↓</button></div><div class="garage-grid">${BIKES.slice().sort((a,b)=>Number(!!a.secret)-Number(!!b.secret)||a.price-b.price).map((b,index) => {
    const owned = save.owned.includes(b.id), equipped = save.bikeId === b.id;
    if(b.secret&&!owned)return '<article class="bike-card secret-vehicle"><p>item ainda não disponível</p></article>';
    return `<article class="bike-card ${equipped ? 'equipped' : ''}"><div class="bike-class">${b.class} / 0${index + 1}</div><h3>${b.name}</h3><img class="bike-image" src="${bikePortrait(b).toDataURL()}" alt="${b.name}, ${b.class.toLowerCase()} vista de lado"/><p>${b.tagline}</p><div class="bike-stats"><div class="bike-stat">VELOCIDADE <strong>${Math.round(b.speed * 3.6)} KM/H</strong></div><div class="bike-stat">${isBicycle(b.id)?'PEDALADAS':'0–100 KM/H'} <strong>${isBicycle(b.id)?'TOQUES REPETIDOS':`${zeroToHundred(b).toFixed(1)} S`}</strong></div><div class="bike-stat curve-stat">CURVAS <strong>${handlingLabel(b.handling)}</strong><meter min="0" max="1.6" value="${b.handling}" aria-label="Agilidade nas curvas: ${handlingLabel(b.handling)}"></meter></div><div class="bike-stat">NITRO <strong>${b.nitroCapacity?`ATÉ ${b.nitroCapacity} CARGAS`:'NÃO USA'}</strong></div><div class="bike-stat">JOELHEIRA <strong>${supportsKneeDown(b.id)?'COMPATÍVEL':'INCOMPATÍVEL'}</strong></div><div class="bike-stat">RESISTÊNCIA DA MOTO <strong>${Math.round(b.armor * 100)}%</strong></div></div><button class="secondary" data-bike="${b.id}" ${equipped || (!owned && save.cash < b.price) ? 'disabled' : ''}>${equipped ? '✓ EQUIPADA' : owned ? 'EQUIPAR ↗' : `COMPRAR · ${money(b.price)}`}</button></article>`;
    ...
    const level = save.upgrades[save.bikeId][k], label = { engine: isBicycle(save.bikeId)?'Transmissão':'Motor', armor: 'Resistência', handling: 'Dirigibilidade' }[k];
...
function launchChampionship(){
  ...
  if(accounts.cache){void startRace(true);return;}
...
async function showChampionshipResult(){
  ...
  beginResult(payoutResult);
  try{
    if(accounts.cache){
      resultPayout=await submitSoloResult();
      $('result-modal').innerHTML=championshipMarkup(save.championship,save,resultPayout?.total);if(resultPayout?.secretUnlocked)showSecretUnlock();if(resultPayout?.policeBikeUnlocked)showPoliceBikeUnlock();showAchievementRewards(resultPayout?.achievements);showKnockdownRewards(resultPayout);$('result-modal').scrollTop=0;return;
    }
    const completed=await finishChampionshipSimulation(completedRace);
    if(recordChampionshipHeat(c,completed)){
      resultPayout=settleRace(save,completedRace,{starterRepair:false,mode:'championship'}) ?? null;
      saveNow();
    }
    $('result-modal').innerHTML=championshipMarkup(c,save,resultPayout?.total);if(resultPayout?.secretUnlocked)showSecretUnlock();if(resultPayout?.policeBikeUnlocked)showPoliceBikeUnlock();showAchievementRewards(resultPayout?.achievements);showKnockdownRewards(resultPayout);$('result-modal').scrollTop=0;
  }catch(e){$('result-modal').innerHTML=`<div class="dialog-body champ-body"><h2>CONFIRMAÇÃO PENDENTE</h2><p>${escapeHTML(e instanceof Error?e.message:'Confira sua conexão.')}</p><button id="retry-result" class="primary">TENTAR NOVAMENTE</button><button id="result-menu-btn" class="secondary">VOLTAR AO MENU</button></div>`;}
  finally{champSettling=false;}
}
function saveChampionshipCheckpoint(){
  if(soloRecorder&&!settled){void checkpointRun();return;}
...
let savingRun:Promise<void>|null=null;
async function checkpointRun(){
  if(savingRun)return savingRun;
  const recorder=soloRecorder,payload=recorder?.payload();if(!recorder||!payload)return;
  savingRun=(async()=>{
    try{
      await queueResult({...payload,account:recorder.accountId,created:Date.now(),finish:false});
      const response=await accounts.api('/checkpoint',payload,45_000);
      if(response.resync)throw Error('A corrida avançou em outro aparelho. Retome pelo menu.');
      recorder.acknowledge(response.cursor);
      if(response.cloud)accounts.acceptCloud(response.cloud,false);
      await removeResult(payload.id);
    }catch(e){if(e instanceof Error && e.message.includes('outro aparelho')){pauseGame();toast(e.message);}}
  })();
  try{await savingRun;}finally{savingRun=null;}
}
async function retryRankedResults() {
  if(retryingRanking || !accounts.session?.account||savingRun)return;
  retryingRanking=true;
  const accountId=accounts.session.account.id;
  try {
    for(const entry of await pendingResults(accountId)){
      if(accounts.session?.account?.id!==accountId)break;
      // Old pre-authority ranking packets cannot become economic rewards.
      if(entry.cursor===undefined){await removeResult(entry.id);continue;}
      try{
        let response=await accounts.api(entry.finish?'/finish':'/checkpoint',entry,45_000);
        if(response.resync){await removeResult(entry.id);toast('A corrida avançou em outro aparelho. Retome o progresso da conta.');break;}
        if(!response.ok){
          const skipped=response.cursor-entry.cursor;
          if(skipped<0)throw Error('Corrida precisa ser retomada.');
          let left=skipped;
          const segments=entry.segments.flatMap(s=>{const cut=Math.min(left,s.count);left-=cut;return cut===s.count?[]:[{...s,count:s.count-cut}];});
          if(left>0)throw Error('Corrida avançou em outro aparelho.');
          response=await accounts.api(entry.finish?'/finish':'/checkpoint',{...entry,cursor:response.cursor,segments},45_000);
          if(!response.ok)break;
        }
        if(response.cloud)accounts.acceptCloud(response.cloud,screen==='menu');
      }catch(e){if(!(e instanceof ApiError)||![400,404,413].includes(e.status))break;toast('O trecho não foi confirmado. Retome a corrida salva na conta.');}
      await removeResult(entry.id);
    }
  }catch{}finally{retryingRanking=false;}
}
async function submitSoloResult():Promise<RacePayout|null>{
  await savingRun;
  const recorder=soloRecorder,payload=recorder?.payload();
  if(!recorder||!payload)return null;
  const entry={...payload,account:recorder.accountId,created:Date.now(),finish:true,abandon:race.result?.reason==='left'};
  await queueResult(entry);
  let response=await accounts.api('/finish',entry,45_000);
  if(response.resync)throw Error('A corrida avançou em outro aparelho. Retome pelo menu.');
  if(!response.ok){recorder.acknowledge(response.cursor);response=await accounts.api('/finish',{...entry,...recorder.payload()},45_000);}
  if(!response.ok)throw Error('Retome a corrida no aparelho em que estava jogando.');
  if(response.cloud)accounts.acceptCloud(response.cloud);
  await removeResult(entry.id);soloRecorder=null;
  return response.payout ?? null;
}
...
async function showResult() {
  ...
  settled = true; screen = 'result'; clearControls();
  if(accounts.cache){
    $('result-modal').innerHTML='<div class="dialog-body"><h2>CONFERINDO A CORRIDA…</h2><p>Confirmando o prêmio e sua garagem.</p></div>';beginResult(race.result);
    try{resultPayout=await submitSoloResult();}
    catch(e){$('result-modal').innerHTML=`<div class="dialog-body"><h2>CONFIRMAÇÃO PENDENTE</h2><p>${escapeHTML(e instanceof Error?e.message:'Confira sua conexão.')}</p><button id="retry-result" class="primary">TENTAR NOVAMENTE</button><button id="result-menu-btn" class="secondary">VOLTAR AO MENU</button></div>`;return;}
  }else{resultPayout = settleRace(save, race) ?? null; saveNow();}
  const r = race.result;
  ...
  const subtitle = r.reason === 'finish' ? r.place <= 5 && getTrack(race.trackId).index < TRACKS.length-1 ? 'Top 5 conquistado. A próxima estrada está liberada.' : 'Dinheiro no bolso. Mais uma história no asfalto.' : raceOutcome(r.reason,{...r,exploded}).description;
  ...
  if(resultPayout && (resultPayout.recordBonus || resultPayout.policeBonus || resultPayout.rivalBonus)){
  ...
  if(resultPayout?.secretUnlocked)showSecretUnlock();if(resultPayout?.policeBikeUnlocked)showPoliceBikeUnlock();showAchievementRewards(resultPayout?.achievements);
  beginResult(r);
}
function resultButtons(multiplayer: boolean, hasNext: boolean) {
  return `${hasNext?`<button class="primary" id="${multiplayer?'online-next-btn':'next-race-btn'}">PRÓXIMA CORRIDA ${icons.arrow}</button>`:''}<button class="${hasNext?'secondary':'primary'}" id="${multiplayer?'online-again-btn':'again-btn'}">CORRER DE NOVO ${icons.arrow}</button>${multiplayer?'<button class="secondary" id="online-lobby-btn">VOLTAR PARA A SALA</button>':''}<button class="secondary" id="${multiplayer?'online-menu-btn':'result-menu-btn'}">${multiplayer?'SAIR DA SALA':'VOLTAR AO MENU'}</button>`;
}
function resetFinish() {
  spectatedId=null;$('spectator-bar').hidden=true;document.body.classList.remove('spectating');
  setText('finish-skip','VER RESULTADO ↗');
  renderer.resetCamera();
  arrestSource=null;$('finish-scene').classList.remove('arrest-scene');$('finish-scene').setAttribute('aria-label','Comemoração na chegada');
...
function beginResult(result:RaceResult) {
  // An account response can fill the result panel while its scene is playing.
  if(finishElapsed!==null)return;
  const finished=result.reason==='finish',arrested=result.reason==='caught',place=result.place;
  clearControls();closeDialogs();paused=false;audio.update(0,false,false,0);
  $('result-modal').classList.toggle('finish-result',finished||arrested);
  if(!finished&&!arrested){screen='result';finishElapsed=null;$<HTMLDialogElement>('result-modal').showModal();return;}
  ...
  if(arrested){
    arrestSource=structuredClone(race);finishOfficer=null;
    $('finish-scene').classList.add('arrest-scene');$('finish-scene').setAttribute('aria-label','Prisão do piloto');
    setText('finish-place',result.arrestCause==='fall'?'QUEDA PERTO DA POLÍCIA':'CAPTURADO PELA POLÍCIA');setText('finish-winner','FIM DA LINHA.');
    $('finish-skip').focus({preventScroll:true});return;
  }
...
function revealResult() {
  ...
  if(onlineMode && arrestSource && beginSpectating())return;
  finishElapsed=Math.max(finishElapsed ?? 0,arrestSource?ARREST_SECONDS:FINISH_SECONDS);screen='result';$('finish-scene').hidden=true;
...
function advanceFinish(dt: number) {
  if(finishElapsed===null)return;
  finishElapsed+=dt;
  if(arrestSource){if(finishElapsed>=ARREST_SECONDS)revealResult();return;}
...
function nextOnlineRace(advance: boolean) {
  online.continue(advance?'next':'again');
}
function update() {
  if (screen === 'menu') {
  ...
  syncSoloNitro();
  if(race.time-champCheckpointAt>=3){champCheckpointAt=race.time;if(soloRecorder)void checkpointRun();else if(championshipMode && save.championship){checkpointChampionship(save.championship,race);saveNow();}}
  ...
  for (const event of race.events) {
    ...
    if (localEvent || event.type==='horn' && Math.abs((race.riders.find(r=>r.id===event.actor)?.z ?? Infinity)-viewingRider().z)<100) audio.event(event);
...
function draw() { renderer.render(race, screen === 'menu',viewingId(),finishElapsed,finishOfficer,arrestSource); if (screen !== 'menu' && finishElapsed===null) { updateHUD(); instruments.draw(race,viewingId(),screen==='spectate'); } updateRecoveryGuide(); }
function frame(now: number) {
  ...
  if(onlineMode) {
    ...
    if(screen==='race' || screen==='spectate'){const p=viewingRider();audio.update(p.speed,!paused && !p.recovery,race.policeActive && Math.abs((race.riders.find(r=>r.profile==='police')?.z ?? 99999)-p.z)<100,race.time,getBike(p.bikeId).style,jumpSoundKey(p),guardRailSoundSide(race.trackId,p),race.trackId==='terra'?'earth':'metal',jumpSoundMaterial(race,p));if(!paused)audio.updateRecovery(p,race.trackId);}
...
function accountGarageAction(button:HTMLButtonElement):GarageAction|null {
  for(const [field,kind] of [['bike','bike'],['helmet','helmet'],['helmetColor','color'],['weapon','weapon'],['kneePad','knee'],['upgrade','upgrade']] as const)
    if(button.dataset[field])return {kind,item:button.dataset[field]};
  const kind:Record<string,GarageAction['kind']>={'buy-nitro':'nitro','repair-btn':'repair','champ-repair':'champRepair','champ-restart':'champRestart','remove-weapon':'removeWeapon','remove-knee':'removeKnee','confirm-reset':'reset'};
  return kind[button.id]?{kind:kind[button.id]}:null;
}
async function changeAccountGarage(action:GarageAction,button?:HTMLButtonElement){
  if(button)button.disabled=true;
  try{
    await accounts.action(action);
...
async function leaveAccountRace(restart=false){
  ...
  startingRace=true;paused=true;clearControls();settled=true;
  finishRider(race,localRider(),'left');
  try{await submitSoloResult();startingRace=false;if(restart)await startRace();else showMenu();}
  catch(e){startingRace=false;settled=false;toast(e instanceof Error?e.message:'Não foi possível confirmar a saída.');}
}
document.addEventListener('click', event => {
  ...
  const action=accounts.cache?accountGarageAction(button):null;
  if(action){void changeAccountGarage(action,button);return;}
  ...
  if(button.dataset.achievementFilter){achievementFilter=button.dataset.achievementFilter as AchievementFilter;showAchievements();return;}
  ...
  switch (button.id) {
    case 'retry-result': settled=false;void showResult();break;
    ...
    case 'spectator-next': case 'online-spectate-btn': cycleSpectator();break;
    case 'spectator-result': showSpectatorResult();break;
    ...
    case 'achievements-btn': case 'view-achievements': achievementFilter='all';showAchievements();break;
    ...
    case 'restart-btn': case 'again-btn': if(championshipMode)break;selectedTrack=race.trackId;selectedCondition=raceCondition(race.condition);if(soloRecorder&&!settled)void leaveAccountRace(true);else void startRace();break;
    ...
    case 'online-lobby-btn': online.continue('lobby');break;
    case 'menu-btn': if(championshipMode){finishRider(race,localRider(),'left');void showResult();break;}if(soloRecorder){void leaveAccountRace();break;}showMenu();break;
...
document.addEventListener('change',event=>{
  ...
  if(element instanceof HTMLSelectElement && element.id==='nitro-bike' && save.owned.includes(element.value)){
    if(accounts.cache){void changeAccountGarage({kind:'bike',item:element.value});return;}
...
window.addEventListener('keydown', e => {
  ...
  if (e.code === 'Escape') { if ((screen === 'race' || screen === 'spectate') && !paused && !document.querySelector('dialog[open]')) { e.preventDefault(); pauseGame(); } return; }
  ...
  if(e.code==='KeyC' && !e.repeat && !paused && onlineMode && ['spectate','finish','result'].includes(screen) && (!document.querySelector('dialog[open]') || $<HTMLDialogElement>('result-modal').open)){e.preventDefault();cycleSpectator();return;}
  if (document.querySelector('dialog[open]')) return;
...
if (testMode) window.__game = {
  ...
  command: (cmd, frames, render=true) => { for (let i = 0; i < frames && race.mode!=='finished'; i++) {soloRecorder?.add(cmd);stepRace(race, { player: cmd });} syncSoloNitro();if (race.mode === 'finished') void showResult();if(render)draw(); },
};
...
function updateRecoveryGuide(){
 const guide=document.getElementById('recovery-guide');if(!guide)return;const p=viewingRider(),f=p.recovery;
 document.body.classList.toggle('on-foot',!!f && screen==='race');guide.hidden=!f || screen!=='race';
 const labels=document.querySelectorAll('.analog-control > span');if(labels[0])labels[0].textContent=f?'ESQUERDA / DIREITA':'VIRAR';
 if(!f)return;
