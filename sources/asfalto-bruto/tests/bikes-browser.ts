...
try{
 await a.goto('http://127.0.0.1:4355/?test');await a.click('#garage-btn');assert.equal(await a.locator('[data-bike]').count(),7);assert.equal(await a.locator('.secret-vehicle').count(),2);assert.ok(await a.locator('[data-bike="lobo"]').isDisabled());
