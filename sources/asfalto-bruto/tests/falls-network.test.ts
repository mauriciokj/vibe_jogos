...
test('protocol14 retains continuous sliding of both bodies without changing snapshots or events',()=>{
 assert.equal(NET_VERSION,14);const {state,presentation}=setup(),received=room(state),raw=JSON.stringify(received);presentation.accept(received,0,10000);
