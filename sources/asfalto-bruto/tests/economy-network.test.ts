...
test('WebSocket account binding prevents forged equipment, refunds failed joins and closes lobby reservations',async()=>{
  ...
  const create={type:'create',version:NET_VERSION,name:'Piloto',trackId:'costa',condition:'day',bikeId:'ferro',loadout:{nitro:5,kneePadId:'gold',weaponId:'chain'},fillBots:true};
  try{
    ...
    const guest=await peer(false);assert.match((await send(guest,{...create,requireAccount:true},'error')).message,/sessão expirou/);
    const gw=await send(guest,{...create,bikeId:'brutal'},'welcome');assert.equal(gw.room.members[0].bikeId,'brutal');assert.equal(gw.room.members[0].nitro,5);assert.equal(gw.room.members[0].kneePadId,'gold');assert.equal(gw.room.members[0].weaponId,'chain');
