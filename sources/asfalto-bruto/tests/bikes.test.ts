...
test('catalog has seven distinct silhouettes and retains all existing ownership IDs and prices',()=>{
 assert.equal(BIKES.length,7);assert.equal(new Set(BIKES.map(b=>b.style)).size,7);
 assert.deepEqual(BIKES.slice(0,3).map(b=>[b.id,b.price]),[['ferro',0],['veneno',2800],['brutal',4800]]);
...
test('agile supermoto holds a tight curve that pushes the chopper onto the shoulder at equal entry speed',()=>{
 const results=BIKES.filter(b=>['falcao','lobo'].includes(b.id)).map(b=>{
  const save=freshSave();save.cash=5000;buyBike(save,b.id);const s=createRace('costa',save);s.riders=s.riders.slice(0,1);s.traffic=[];s.obstacles=[];s.mode='racing';
...
test('buying and upgrading each model equips its actual attributes and never charges twice',()=>{
 const save=freshSave();assert.equal(buyBike(save,'lobo'),false);assert.equal(save.bikeId,'ferro');save.cash=50_000;
...
test('version-one campaign saves retain cash, old bikes, upgrades and new purchases after reload',()=>{
 ...
 try{
  const old=freshSave();old.cash=16000;buyBike(old,'veneno');buyUpgrade(old,'engine');old.unlocked=2;old.records.costa={time:150,place:2};persist(old);
  const restored=loadSave();assert.deepEqual(restored,old);buyBike(restored,'lobo');buyUpgrade(restored,'handling');persist(restored);assert.deepEqual(loadSave(),restored);
