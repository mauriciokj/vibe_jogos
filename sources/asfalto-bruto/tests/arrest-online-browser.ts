...
try{
 ...
 await a.click('#finish-skip');assert.equal((await state()).screen,'spectate');assert.equal((await state()).spectator.id,bid);await a.click('#spectator-result');assert.match(await a.locator('.result-sub').innerText(),/30 metros/);await a.click('#online-menu-btn');await a.locator('#menu').waitFor();assert.equal((await state()).arrest,null);assert.equal((await state(b)).screen,'race');
