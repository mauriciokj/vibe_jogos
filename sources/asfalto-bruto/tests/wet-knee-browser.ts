...
for(const p of [a,b]){
  ...
  await p.route('**/api/visitors*',route=>route.fulfill({json:{visitors:0,since:'2026-09-08T00:00:00Z'}}));
}
...
try {
  ...
  await reset();await a.keyboard.press('d');await a.keyboard.down('d');await a.waitForFunction(()=>JSON.parse(window.render_game_to_text()).player.wetKneeTime>.8);
  const before=(await store.read(code))!.race!.riders.find(r=>r.id===aid)!;assert.ok(before.wetKneeTicks!>40);assert.equal(before.falls,0);
  ...
  assert.deepEqual(errors,[]);const report={ok:true,shortContactSafe:true,liftResets:true,reconnectKeepsExposure:true,authoritativeDelayedFall:true,wetLimitSeconds:2,humans:2,bots:6,errors};await fs.writeFile(`${folder}/online.json`,JSON.stringify(report,null,2));console.log(report);
