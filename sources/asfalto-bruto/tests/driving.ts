...
export function safeDrivingCommand(s: RaceState): Command {
  ...
  const cost = (x: number) => Math.abs(x - p.x) * .8 + (x < 0 ? .4 : 0)
    + hazards.reduce((sum, t) => sum + (Math.abs(x - t.x) < (s.trackId==='terra'?trafficShape(s.trackId,t.kind==='tractor'?'tractor':'car').contact+.4:t.kind==='truck'?3.15:2.65) ? 100 - Math.max(0, t.z - p.z) * .2 : 0), 0)
