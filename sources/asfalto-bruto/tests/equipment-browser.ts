...
try{
  ...
  await a.click('#garage-btn');await a.click('[data-bike="ferro"]');await a.click('[data-close="garage-modal"]');await a.click('[data-route="costa:rain"]');await a.click('#start-btn');await fixture(a,1000);
  await a.evaluate(()=>{const s=JSON.parse(window.__game!.snapshot());s.trackId='porto';s.riders=s.riders.slice(0,1);window.__game!.restore(JSON.stringify(s));});
  await a.keyboard.press('d');await a.keyboard.down('d');await a.evaluate(()=>window.advanceTime(17));await a.keyboard.up('d');assert.equal((await state()).player.falls,0);
  await a.evaluate(()=>window.__game!.command({throttle:0,brake:0,steer:.25,attack:null},179));assert.equal((await state()).player.falls,0);assert.equal((await state()).player.wetKneeTime,3);await shot('rain-knee-safe');
