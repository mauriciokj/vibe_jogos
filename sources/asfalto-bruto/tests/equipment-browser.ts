...
try{
  ...
  await a.evaluate(()=>window.__game!.command({throttle:0,brake:0,steer:.25,attack:null},119));assert.equal((await state()).player.falls,0);assert.equal((await state()).player.wetKneeTime,2);await shot('rain-knee-safe');
